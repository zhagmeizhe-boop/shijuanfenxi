...
VISUAL_SIGNAL_PATTERN = re.compile(
    r"(?:如图|下图|下表|统计图|折线图|柱状图|扇形图|表格|三角形|长方形|正方形|圆|圆柱|圆锥|展开图|截面|蝴蝶模型|燕尾模型|一半模型|鸟头|沙漏|相似|割补|等高|共边|格点|三视图|水中浸物|长度计算|角度计算|图形变换)"
)
DIM2_TEXT_SIGNAL_PATTERN = re.compile(
    r"(?:如图|下图|三角形|长方形|正方形|平行四边形|梯形|圆|扇形|立方体|正方体|长方体|圆柱|圆锥|展开图|截面|视图|辅助线|折叠|旋转|平移|对称|蝴蝶模型|燕尾模型|一半模型|鸟头|沙漏|相似|割补|等高|共边|面积比|格点|三视图|水中浸物|长度计算|角度计算|标向|多边形内角和|图形变换)"
)
