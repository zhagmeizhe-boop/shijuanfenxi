...
请只为 dim4「实践创新」生成可审计画像。新版 dim4 先识别题目所属知识点，再判断它在该知识点内部处于 L1-L5 哪个创新/变式等级。
dim4 不评知识广度、年级、是否奥数/竞赛、计算量、题干长度或完整逻辑链长度。
只返回一个 JSON 对象，不要 Markdown，不要解释。"""


USER_PROMPT_TEMPLATE = """请为下面这道高思导引参考题生成 dim4 策略画像。
...
- L1：该知识点基础模板题，方法直接。
- L2：轻度变式，问法或条件表达有小变化。
- L3：中度变式，需要识别题型变形、选择策略或一次结构重组。
- L4：高阶变式，需要构造、试探、分类、反向思考或非套路表征。
- L5：该知识点内压轴级题，需要全局设计、多策略组合、开放探索或强约束收束。
